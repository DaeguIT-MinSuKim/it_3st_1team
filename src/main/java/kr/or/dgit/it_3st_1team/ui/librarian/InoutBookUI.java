...
public class InoutBookUI extends JPanel implements ActionListener, KeyListener {
	...
	private Object[][] getRow() {
		Object[][] rows = new Object[][] { { null, null, null, null, null, null, null, null },
	...
	protected void actionPerformedBtnUserSearch(ActionEvent e) {
		...
		if (!(tfUserCode.getText().equals(""))) {
			 ...
			 * else { JOptionPane.showMessageDialog(null, "회원코드를 입력해주세요"); }
			 */
	}
	...
	private void inoutBarcord() {
		if (tfBkCode.getText().length() == 19) {
			...
			user.setCode(tfUserCode.getText());
			Takeinout inout = new Takeinout();
			ManageBookService bService = new ManageBookService();
			book = bService.selectBookByCodeWithAPI(book);
			inout.setBook(book);
			inout.setUser(user);
			TakeinoutService service = new TakeinoutService();
			Takeinout findinout = service.selectTakeinoutByBkCode(inout);
			Takeinout outBook = service.selectMatchingBook(book);
			List<Reserve> list = searchReserve(inout);
			if (findinout == null) {
				if (outBook == null) {
					try{
						if (list.get(0).getCode().equals(user.getCode())) {
							...
							ReserveService.getInstance().deleteReserveBookWithAPI(list.get(0));
							return;
						} else if (!(list.get(0).getCode().equals(user.getCode()))) {
							JOptionPane.showMessageDialog(null, list.get(0).getCode() + "님께서 예약한 도서 입니다. 대여할수 없습니다.");
							return;
						}
	...
	private List<Reserve> searchReserve(Takeinout inout) {
		Reserve res = new Reserve();
		res.setBkcode(inout.getBook().getBkCode());
		List<Reserve> list = ReserveService.getInstance().selectReserveBookByBkCode(res);
		return list;
	}

	// 대여
	protected void actionPerformedBtnOut(ActionEvent e) {
		if (tfBkCode.getText().length() == 18) {
			btnInoutAction();
		} else {
			JOptionPane.showMessageDialog(null, "도서코드를 정확하게 입력해주세요");
		}
	}
	...
	private void btnInoutAction() {
		String str = tfBkCode.getText();
		Book book = new Book();
		book.setBkCode(str);
		User user = new User();
		user.setCode(tfUserCode.getText());
